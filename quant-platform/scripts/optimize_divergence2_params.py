    ...
    params.update(
        {
            ...
            "day1_max_volume_ratio": round(rng.uniform(2.0, 8.0), 2),
            "range_min_amplitude_30": round(rng.uniform(0.05, 0.60), 3),
            "range_min_return_20": round(rng.uniform(-0.15, 0.30), 3),
            "day2_min_pct_chg": round(rng.uniform(-8.0, 2.0), 1),
            "day2_max_pct_chg": round(rng.uniform(3.0, 12.0), 1),
            "day2_max_volume_ratio": round(rng.uniform(1.0, 3.5), 2),
            "day2_min_close_position": round(rng.uniform(0.20, 0.90), 2),
            "day2_max_upper_shadow": round(rng.uniform(0.02, 0.18), 3),
            "day2_min_close_vs_day1_close": round(rng.uniform(0.88, 1.08), 3),
            "entry_min_open_gap_pct_chg": round(rng.uniform(0.0, 3.5), 1),
            "entry_max_open_gap_pct_chg": round(rng.uniform(3.0, 10.0), 1),
            "entry_min_high_from_open_pct_chg": round(rng.uniform(1.5, 7.0), 1),
        }
    )
    return params
